...
var bulletSpeed = 0; //How fast the bullets should move
...
function Bullet(xDir, yDir, x, y){
  ...
  this.x = x;
  this.y = y;
  this.xVel = bulletSpeed;//Replace bulletSpeed with some tricky ricky stuff about calculating the velocities with the direction
...
io.sockets.on('connection',function(socket){
  ...
  socket.on('newBullet',function(data){
    if(balls[socket.id]){
      ...
      }else{data = {x:null, y:null};}
      new Bullet(1, 1, 150, 150);
    }
  });
...
setInterval(function(){
  for(var id in balls){
    balls[id].move();
    for(var id in bullets){bullets[id].move();}
  }
  io.sockets.emit('move',{'balls':balls,'bullets':bullets});
  frames++;
},1000/fps);
setInterval(function(){
  if(frames<55){console.log('Server is lagging - FPS: '+frames);}
  frames = 0;
},1000);