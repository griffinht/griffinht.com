...
var bulletCollisions = true; //Should the server check if each bullet is hitting each ball?
var balls = {};
var bullets = {};
var colors = ['red','blue'];
    ...
    if(bulletCollisions){
      for(var id in bullets){
        if(bullets[id].parentId!=this.id){
          ...
          if(distance<=(ballImage.height+bulletImage.height)/2 || distance<=(ballImage.width+bulletImage.width)/2){
            delete bullets[id];
